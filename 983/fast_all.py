...
def check_M_fast(r2):
    ...
    # Pre-generate subset sum combinations
    # Since we just want to avoid any sum of length 4, 6, 8, 10 equaling u - v
    D = set()
    for u in pts:
        for v in pts:
    ...
    for comb in combs:
        # Check if any even length subset sum with coefficients in {-1, 0, 1}
        # and length >= 4 is in D
